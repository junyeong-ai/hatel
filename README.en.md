...
- The **`memory`** section is which instruction files entered context, by their path in the repository (`~/…` outside it). `load_reason` says why: `session_start` and `compact` repeat for every file each time context is rebuilt, while `path_glob_match` and `nested_traversal` carry `trigger_file_path` (the file whose access pulled the rule in) and the turn (`prompt_id`) they happened under. A file the context already holds is not loaded again, so a missing load does not mean the file was absent.
