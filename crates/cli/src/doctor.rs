...
fn report_hooks(files: &[cs::ScopeFile]) -> bool {
    ...
    if covered.len() == total {
        println!("  ✓ all {total} lifecycle events invoke `{hook_cmd}`");
        return true;
    }

    // Some honored events are wired but not all — report the real gap first, so partial coverage
    // is never masked by the "blocked" message below.
    if !covered.is_empty() {
    ...
    // Nothing honored is wired.
    if cs::hook_wired_but_blocked(files) {
        println!(
            "  ✗ a hook invokes `{hook_cmd}` but is BLOCKED by allowManagedHooksOnly — \
             deploy it as a MANAGED hook (IT/MDM) or no events are captured"
        );
        return false;
    }
    println!("  ✗ no hook invokes `{hook_cmd}` — events are not captured");
    if managed_only {
