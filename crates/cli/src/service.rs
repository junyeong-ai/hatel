...
fn linux(exe: &Path, remove: bool, print: bool) -> i32 {
    ...
    quiet(Command::new("systemctl").args(["--user", "daemon-reload"]));
    quiet(Command::new("systemctl").args(["--user", "enable", &svc])); // start on login
    // `restart` is the operative step: it starts the unit if stopped and, crucially, repoints one
    // that's already running to the freshly-written ExecStart — so re-running after a move works.
    match Command::new("systemctl").args(["--user", "restart", &svc]).status() {
        Ok(s) if s.success() => {
        ...
        Ok(s) => {
            eprintln!("service: `systemctl --user restart` exited {s}; the unit is at {}", path.display());
            1
        }
        Err(e) => {
