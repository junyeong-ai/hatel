...
> 키는 알파벳 순으로 직렬화됩니다. 위는 `Bash` 그룹만 보였고, 실제 리포트엔 `Edit·Grep·Read`가 같은 형태로 이어집니다. 그 차원의 값을 아예 싣지 않은 기록의 그룹은 `key`가 `null`입니다(텍스트·마크다운 뷰의 `—` 행). 필드가 없는 것과 어떤 값이 있는 것을 글자가 아니라 모양으로 구분할 수 있습니다. `unreadable_kinds`가 `null`이 아니면 **로드된 스키마가 선언하지 않는 Kind가 원장에 있다**는 뜻입니다 — 이 집계는 수집된 것보다 적게 답한 것이고, 이름과 고칠 위치가 거기 실려 옵니다([커스텀 지표](#커스텀-지표-플러그인) 참조).
