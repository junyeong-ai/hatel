...
| Binary | Role |
|---|---|
| `hatel-hook` | wired into `settings.json` hooks; reads one event on stdin, records it, exits. No async runtime — ~3 ms cold start. |
...
`http/json` is required: this receiver decodes the JSON OTLP encoding so it needs
no protobuf dependency. Putting the block at the **user** level feeds one collector
from every project; the receiver still shows only the current project by default.
...
- A signal that only your project's own logic knows → **`emit`** (your tooling
  records it; the harness cannot observe it).
...
harness can't. `plugins/example.toml` is a worked example: a CI-check rollup recorded
