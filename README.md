...
- **`memory`** 섹션은 어떤 지침 파일이 컨텍스트에 들어왔는지를 저장소 안 경로(저장소 밖이면 `~/…`)로 보여줍니다. `load_reason`이 이유입니다. `session_start`와 `compact`는 컨텍스트를 다시 세울 때마다 모든 파일에 대해 반복되고, `path_glob_match`와 `nested_traversal`은 규칙을 불러들인 파일(`trigger_file_path`)과 그때의 턴(`prompt_id`)을 함께 싣습니다. 컨텍스트가 이미 가진 파일은 다시 로드되지 않으므로, 로드 기록이 없다고 파일이 없었던 것은 아닙니다.
