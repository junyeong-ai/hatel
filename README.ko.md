...
서브에이전트별 뷰로 결합합니다. 호스팅할 대시보드가 없고, 데이터는 머신을 떠나지 않습니다.

- **네이티브 OTel** (push) 은 머신 신호를 운반합니다: 토큰, 비용, 활성 시간, 라인 수, 그리고
  `agent.name`을 통한 서브에이전트별 귀속. 와이어 상에 프로젝트 정보가 없으므로 세션 인덱스를
  통해 프로젝트에 결합됩니다.
- **훅** (event) 은 프로젝트 컨텍스트(`cwd`)와 OTel이 표현할 수 없는 도메인 이벤트를 운반합니다:
  도구 호출, 프롬프트 크기, 메모리 로드, 서브에이전트 종료, 컴팩션 — 그리고 플러그인이 정의하는
  무엇이든.
...
| 바이너리 | 역할 |
|---|---|
| `hatel-hook` | `settings.json` 훅에 연결됨. stdin으로 이벤트 하나를 읽고 기록한 뒤 종료. 비동기 런타임 없음 — 콜드 스타트 ~3 ms. |
...
`http/json`은 필수입니다: 이 리시버는 JSON OTLP 인코딩을 디코드하므로 protobuf 의존성이 필요
없습니다. 블록을 **user** 레벨에 두면 하나의 컬렉터가 모든 프로젝트로부터 데이터를 받습니다.
...
인스턴스는 종료되고, 비용 스냅샷은 정확히 하나의 라이터만 갖습니다. JSON이 아니거나 프로토콜이
틀린 본문은 `400`으로 거부하고(잘못 설정된 익스포터가 조용한 `200`이 아니라 실제 신호를 받도록),
요청 본문을 64 MB로 제한하며, 손상된 락은 크래시 대신 복구합니다. 영속 상태는 보존 기간
(`HATEL_RETENTION_DAYS`)으로 제한됩니다. 메모리 상의 세션별 누산기는 프로세스 시작
이후 관측된 세션 수로 제한되며, 정상적으로 재시작하는 데몬은 이를 작게 유지합니다.
...
| `init [--scope user\|project\|local] [--print] [--remove]` | `settings.json`에 텔레메트리 env + 라이프사이클 훅을 연결(또는 해제) — 멱등적·비파괴적·원자적. `--print`는 managed settings용 블록을 출력. |
...
- Claude Code 라이프사이클이 관측할 수 있는 신호 → **훅 바인딩**(코드 0, 세션의 프로젝트로 자동
  귀속).
- 프로젝트 자체 로직만 아는 신호 → **`emit`**(당신의 도구가 기록함. 하니스는 관측할 수 없음).
...
하니스는 모릅니다. `plugins/example.toml`은 동작하는 예시입니다: 당신의 도구가 기록하는 CI 검사
